...
Where FILENAME is the name of your config file. the file MUST be located inside the Config dir..
...
Do it like this. Bec.exe -f foo_bar.cfg 

--dec switch will disable external scripts, this applys to both Commands.xml and Scheduler.xml
This means that no bat or cmd files may be executed by these files.


--dsc switch will disable the wait & check for the arma server. 
Bec will try and connect directly to the rcon regadless if the server is running or not.
...
NOTE: Bec might need admin permission to work properly. altho when using the --dsc it should not require it.




Are you Upgrading from a previouse version ???. 
...
For Bec to work properly it depends on Battleye and a stabile internet connection and ofcource the Arma server iteslf.
...
You need to verify that you have a BEServer.cfg . if its missing you need to create this file for Batteye before Bec can start to work.
...
Example: Server total limit is 50 and you set SlotLimit = 45 
This means 45 normal players can connect. the 5 remaining need to be registered users. normally in admin.xml or users.xml
So in other words. max 45 normal player + 5 registered players.
